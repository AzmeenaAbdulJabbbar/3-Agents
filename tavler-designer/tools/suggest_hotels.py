...
async def suggest_hotels(input: HotelData) -> dict:
    ...
    user_budget = input["budget"].lower().strip()
    ...
    # Mock hotel data for each budget
    mock_hotels = {
        "low": [
    ...
    if user_budget not in mock_hotels:
        return {
            "message": (
    ...
    return {
        "city": city_name,
        "budget_range": budget_ranges[user_budget],
        "hotels": mock_hotels[user_budget]
    }
